    ...
    for i in range(num_objects):
        if idx >= 0 and args.load_scene:
            ...
            while True:
                ...
                for (objobj, xx, yy, hh, rr, tt) in positions:
                    box_x, box_y, _ = obj_info['info_box'][obj_name]
                    box_xx, box_yy, _ = obj_info['info_box'][objobj]
                    if check(xx,yy,box_xx,box_yy,rr*1.1,tt,x,y,box_x,box_y,r*1.1,theta) and hh == h:
                        margins_good = False
                        break
